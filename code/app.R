...
ui <- fluidPage(
  ...
  sidebarLayout(
    sidebarPanel(
      ...
      radioButtons("PIR", "Select Poverty Income Ratio:",
                   choices = c(">= 2" = 1, "< 2" = 0))
    ),
    mainPanel(
      plotOutput("BMIPlot"),
...
server <- function(input, output) {
  ...
  # Reactive expression for the model fit
  fit <- reactive({
    glm(bmi ~ as.factor(FFQ0102) + INDFMPIR, data = filtered_data(), family = binomial(link = "logit"))
  })
  
  # Reactive expression for specific odds
  specific_odds <- reactive({
    specific_data <- data.frame(FFQ0102 = as.numeric(input$frequency), INDFMPIR = as.numeric(input$PIR))
    exp(predict(fit(), newdata = specific_data, type = "response"))
  })
  
  # Create the plot automatically when inputs change
  ...
  # Display specific odds automatically when inputs change
  output$specificOdds <- renderText({
    paste("The estimated odds for the selected frequency and PIR is:", specific_odds())
  })
}
